...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 27 19:11:02 2025
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(conv1_in_ch, conv1_out_ch, kern1_size, groups = group_size, padding = kern1_size)   # (kern1_size//2 + 1) , kern1_size
        self.conv2 = nn.Conv1d(conv2_in_ch, conv2_out_ch, kern2_size, padding = kern2_size)  # (kern2_size//2 + 1) , kern2_size
    ...
    def forward(self, x):
        ...
        x = self.elu(self.conv1(x))  # self.maxpool1d_1(
        # print('cool2')
        x = self.elu(self.conv2(x))  # self.maxpool1d_2(
        # print('cool3')
    ...
    def get_output_dim(self, input_shape): # function to get the flattened output size of the convolutional layers so that it can be passed to the fc1 definition
        ...
        with torch.no_grad():
            ...
            output = dummy_input
            output = self.elu(self.conv1(output))  # self.maxpool1d_1(
            output = self.elu(self.conv2(output))  # self.maxpool1d_2( 
...
# =============================================================================
    ...
    for inputs, target, v0s, sigs, x0s, pots in train_loader:
        ...
        loss =  MSE + torch.sum(mu_dep_loss**2)/(1000000) # + sum_theta
...
# =============================================================================
                ...
                if((sigs[j]==0.05) and x0s[j]==0.5):
                    ...
                    axq.legend()
                    plt.tight_layout()
                    plt.savefig('ML_funcderiv_9_6_pt8_narrow.pdf')
                    plt.show()
                ...
                if((sigs[j]==0.15) and x0s[j]==0.5):
                    ...
                    axq.legend()
                    plt.tight_layout()
                    plt.savefig('ML_funcderiv_9_6_pt8_wide_10epochs.pdf')
                    # plt.savefig('ML_funcderiv_9_6_pt8_wide.pdf')
                    plt.show()
                    print(x0s[j], v0s[j], sigs[j])
...
plt.tight_layout()
plt.savefig('scatter_in_9_6_pt8.pdf')
plt.show()
...
# ax3.set_zticklabels([])
plt.tight_layout()
plt.savefig('surface_in_9_6_pt8.pdf')
plt.show()
...
ax6.grid()
plt.tight_layout()
plt.savefig('swarm_in_18_6_pt8.pdf')
plt.show()
...
ax7.grid()
plt.tight_layout()
plt.savefig('line_in_18_6_pt8.pdf')
plt.show()
...
ax8.grid()
plt.tight_layout()
plt.savefig('spreadoutx_in_18_6_pt8.pdf')
plt.show()
...
ax9.grid()
plt.tight_layout()
plt.savefig('avgtrainv_in_9_6_pt8.pdf')
plt.show()
...
ax10.legend()
plt.tight_layout()
plt.savefig('train_test_curve1_in_9_6_pt8.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('parity_plot_in_9_6_pt8.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('mosaic_in_9_6_pt8.pdf')
plt.show()
...
# ax13.legend()
plt.tight_layout()
plt.savefig('theta_vs_epochs_in_9_6_pt8.pdf')
plt.show()
...
# ax3.set_zticklabels([])
plt.tight_layout()
plt.savefig('theta_surface_in_9_6_pt8.pdf')
plt.show()
...
#         #         
#         #         mu_array = np.array([])
#         #         for j in range(len(funcderiv)):
#         #             mu1 = simpson((funcderiv+pots)[j].detach(), x=well)
#         #             mu_array = np.append(mu_array,mu1)
#         #         mu_array = torch.FloatTensor(mu_array)
#         #         mu_array = mu_array.unsqueeze(1)
#         # =============================================================================
...
#         
#         
#         if (((epoch+1)%epoch_mod==0) and (i==0)):
#             print('=============================================================================')
#             print(f'Epoch[{epoch+1}/{n_iters}]')
#             print('### TRAIN ###')
#             # print(f'\ncos(theta) loss = {(torch.sum(cos_theta)/len(cos_theta))}')
...
#         train_loss_array[epoch] += MSE/len(train_loader) ## for train test curve later    
#         
#         loss =  MSE + torch.sum(mu_dep_loss**2)/5000000
#         # loss = criterion(outputs.squeeze(),target) + theta_scalar*sum_theta
#         
#         loss.backward() 
#         optimizer.step() 
#         
...
#             figq = plt.figure(figsize = (9,6))
#             axq = figq.add_subplot(111)
#             axq.plot(x_array,funcderiv[j].detach(), label=f'ML, mu={mu_array[j]}', color='royalblue',linewidth=2.5)
#             axq.plot(x_array, -pots[j]+funcderiv[j,5].detach(), label='Ideal', color='darkviolet',linewidth=2.5)
#             axq.set_ylabel(r'$\frac{\delta T[\rho]}{\delta \rho}$')
#             axq.set_xlabel(r'$\rho(x)$')
#             axq.grid()
#             axq.legend()
