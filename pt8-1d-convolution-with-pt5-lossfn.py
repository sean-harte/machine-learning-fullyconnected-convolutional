...
loss is MSE and mu equation. still multiplying mu by the ML funcderiv
'''
...
class NeuralNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(conv1_in_ch, conv1_out_ch, kern1_size, groups = group_size, padding = (0*kern1_size - 3))   # (kern1_size//2 + 1)
        self.conv2 = nn.Conv1d(conv2_in_ch, conv2_out_ch, kern2_size, padding = 0*kern2_size)  # (kern2_size//2 + 1)
...
# class NeuralNet(nn.Module):
    def __init__(self):
        super(NeuralNet,self).__init__()
        self.conv1 = nn.Conv1d(conv1_in_ch, conv1_out_ch, kern1_size, groups = group_size)
        self.conv2 = nn.Conv1d(conv2_in_ch, conv2_out_ch, kern2_size)
        self.maxpool1d_1 = nn.MaxPool1d(maxpool_kern1, maxpool_stride1) # default stride is kernal size for MaxPool1d
        self.maxpool1d_2 = nn.MaxPool1d(maxpool_kern2, maxpool_stride2)
    ...
    def forward(self, x):
        ...
        # print('cool1')
        #print(f"Input shape before conv1 after unsqueeze: {x.shape}")
        x = self.maxpool1d_1(self.relu(self.conv1(x))) # self.maxpool1d_1(
        # print('cool2')
        x = self.maxpool1d_2(self.relu(self.conv2(x)))  # self.maxpool1d_2(
        # print('cool3')
        # print(f'Before .view = {x.size()}')
        # print(x)
        # print(self.fc1_in)
    ...
    def get_output_dim(self, input_shape): # function to get the flattened output size of the convolutional layers so that it can be passed to the fc1 definition
        ...
        with torch.no_grad():
            ...
            output = self.maxpool1d_1(self.relu(self.conv1(output)))  
            output = self.maxpool1d_2(self.relu(self.conv2(output)))  
            flattened_size = output.numel() // output.shape[0]  # Compute size per sample
        return flattened_size
...
# =============================================================================
    ...
    for inputs, target, v0s, sigs, x0s, pots in train_loader:
        ...
        # funcderiv = funcderiv_scalar*funcderiv # funcderiv_scalar
        ...
        loss =  criterion(outputs.squeeze(),target) + torch.sum(mu_dep_loss**2)/10000
...
plt.tight_layout()
plt.savefig('scatter_in_9_6_pt7.pdf')
plt.show()
...
# ax3.set_zticklabels([])
plt.tight_layout()
plt.savefig('surface_in_9_6_pt7.pdf')
plt.show()
...
ax6.grid()
plt.tight_layout()
plt.savefig('swarm_in_18_6_pt7.pdf')
plt.show()
...
ax7.grid()
plt.tight_layout()
plt.savefig('line_in_18_6_pt7.pdf')
plt.show()
...
ax8.grid()
plt.tight_layout()
plt.savefig('spreadoutx_in_18_6_pt7.pdf')
plt.show()
...
ax9.grid()
plt.tight_layout()
plt.savefig('avgtrainv_in_9_6_pt7.pdf')
plt.show()
...
ax10.legend()
plt.tight_layout()
plt.savefig('train_test_curve1_in_9_6_pt7.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('parity_plot_in_9_6_pt7.pdf')
plt.show()
...
plt.tight_layout()
plt.savefig('mosaic_in_9_6_pt7.pdf')
plt.show()
...
# ax13.legend()
plt.tight_layout()
plt.savefig('theta_vs_epochs_in_9_6_pt7.pdf')
plt.show()
...
# ax3.set_zticklabels([])
plt.tight_layout()
plt.savefig('theta_surface_in_9_6_pt7.pdf')
plt.show()
